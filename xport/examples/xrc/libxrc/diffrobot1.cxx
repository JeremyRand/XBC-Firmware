...
CDiffRobot1::CDiffRobot1(unsigned int translationScale, unsigned int rotationScale, int relativeScale) :
	CDiffBase(translationScale, rotationScale, 0),
	CBluetoothQueue(0) 
	{
	...
	CDiffBase::SetInterruptCont(&m_intCont);
	...
	SetCalibrate(0, 2048 + relativeScale); // higher, turn right, lower, turn left

	if (m_menu.GetInput())
		m_menu.Execute();
	}

CDiffRobot1::~CDiffRobot1()
	{
	}

CInterruptCont *CDiffRobot1::GetInterruptCont()
	{
	return &m_intCont;
	}
...
void ShowMotorPositions(int val, int arg0, int arg1)
	{
	CDiffRobot1 *probot = (CDiffRobot1 *)val;
	...
	while(!CMenu::Return())
		{
		CMenu::Printxyf(4, 4, 8, false, "%d", probot->GetPosition(TRANS_AXIS));
		CMenu::Printxyf(18, 4, 8, false, "%d", probot->GetPosition(ROTATE_AXIS));
		CMenu::Printxyf(18, 8, 8, false, "%d", probot->CAxesOpen::GetPosition(0));
		CMenu::Printxyf(4, 8, 8, false, "%d", probot->CAxesOpen::GetPosition(1));
		}
	}
...
void HoldMotorPositions(int val, int arg0, int arg1)
	...
	probot->SyncTrajectory(TRANS_AXIS);
...
void MoveForward(int val, int arg0, int arg1)
	...
	probot->Move(TRANS_AXIS, probot->GetPosition(TRANS_AXIS)+10000, 1800, 1800);
	probot->Execute();
	CMenu::Printxyf(11, 6, 0, false, "Translation");
	while(!probot->Done(TRANS_AXIS))
		CMenu::Printxyf(11, 7, 8, false, "%d", probot->GetPosition(TRANS_AXIS));
	}
