...
CTone::_SetEnableReg(bool enable)
{
 ...
 int CTone::Play(char *data, int size, bool munchbool, bool fastbool)
 	{
	...
	if(munchbool) m_munch_data=data;
	if(munchbool) m_munch_size=size;
 ...
 void CTone::Interrupt(unsigned char vector)
	...
	{
		Stop();
		Play(m_munch_data,m_munch_size,true,true);
	}else Stop();
