...
void draw_blob(Image &dest, CBlob *blob, ushort color,
               bool showseg, bool showell)
{
  //ushort white = 0xffffff;
  ushort accent_color = color;
  SMomentStats stats;
...
void image_blobify(const Image &src, CBlobAssembler &bass, 
		   bool recordSegments, int row_lim, int col_lim, CVision *pVision)
{
  SSegment seg;
  ...
  pVision->LockSegmentBuffer(&bufIndex);
  len = pVision->m_segments[bufIndex];
  segmentBuffer = pVision->m_segmentBuffer[bufIndex];
 // printf("* %d\n", len);
  for (i=0; i<len; i++)
	  {
	  segment = segmentBuffer[i];
//	  model = segment & 0x07;
	  segment >>= 3;
	  seg.row = (segment&0x1ff)>>1;
	  segment >>= 9;
	  seg.startCol = (segment&0x3ff)>>1;
	  segment >>= 10;
	  seg.endCol = (segment&0x3ff)>>1;
//	  printf("%d %d %d\n", seg.row, seg.startCol, seg.endCol); 
	  bass.Add(seg);
	  }
#endif
...
{
  SMomentStats stats;
  ushort white = Image::make_rgb(0xff, 0xff, 0xff);
  ushort color;
  int i=0;
