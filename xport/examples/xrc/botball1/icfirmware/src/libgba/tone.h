...
public:
  ...
  int Play(char *data, int size, bool munchbool=false, bool fastbool=false, int loops = -1, bool in_middle = false);
  void Stop();
  bool Playing();

  void SetVolumeA(short volume);
  
  int PlayB(char *data, int size, bool munchbool=false, bool fastbool=false, int loops = -1, bool in_middle = false);
  void StopB();
  bool PlayingB();
  void SetVolumeB(short volume);
...
protected:
  ...
  bool m_playing_no_int;
	...
	char * m_munch_data_b;
	bool m_playing_munch_b;
	int m_munch_size_b;
	bool m_munch_fast_b;
	int m_munch_loops_b;
	
	char * m_remaining_addr;
	char * m_remaining_addr_b;
	int m_remaining_samples;
	int m_remaining_samples_b;
	bool m_remaining_fast;
	bool m_remaining_fast_b; 
	
	/*
	int m_playlist_last_index_a;
	int m_playlist_last_index_b;
	(char *) m_playlist_data_a[10];
	(char *) m_playlist_data_b[10];
	int m_playlist_size[10];
	bool m_playlist_fast[10];
	bool m_playlist_munch_a[10];
	bool m_playlist_munch_b[10];
	int m_playlist_loops_a[10];
	int m_playlist_loops_b[10];
	*/
