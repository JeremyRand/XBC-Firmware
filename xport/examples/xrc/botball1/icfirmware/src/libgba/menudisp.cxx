...
void CMenuDisp::DrawMenu()
{
  ...
  // Added by Jeremy to allow scrolling
  short start_item;
  if(m_menuElement->GetNumMenuItems() < 10)
  {
	start_item = 0;
  }
  else
  {
	start_item = m_menuElement->GetCurrentItemIndex()  - 4; // Start 4 above current selection
	if(start_item < 0) // Make sure no blank space at top
	{
		start_item = 0;
	}
	if(start_item > m_menuElement->GetNumMenuItems() - 10) // Make sure no blank space at bottom
	{
		start_item = m_menuElement->GetNumMenuItems() - 10;
	}
  }
  
  //for(unsigned short i = 0; i < m_menuElement->GetNumMenuItems(); ++i)
  for(unsigned short i = (unsigned short) start_item; i < m_menuElement->GetNumMenuItems() && i < start_item + 10; ++i)
    {
      bool rev = false;
