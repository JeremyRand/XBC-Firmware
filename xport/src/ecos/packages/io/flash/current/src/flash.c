...
#ifdef __thumb__  // deal with ARM/THUMB odd address function boundaries
		if ((long)(&flash_query)&0x01)
		{
			memcpy(_flash_query, (char *)(&flash_query)-1, code_len+1);
			(char *)_flash_query = (char *)_flash_query + 1;
		}
		else
#endif
...
#ifdef __thumb__  // deal with ARM/THUMB odd address function boundaries
		if ((long)(&flash_erase_block)&0x01)
		{
			memcpy(_flash_erase_block, (char *)(&flash_erase_block)-1, code_len+1);
			(char *)_flash_erase_block = (char *)_flash_erase_block + 1;
		}
		else
#endif
...
#ifdef __thumb__  // deal with ARM/THUMB odd address function boundaries
		if ((long)(&flash_program_buf)&0x01)
		{
			memcpy(_flash_program_buf, (char *)(&flash_program_buf)-1, code_len+1);
			(char *)_flash_program_buf = (char *)_flash_program_buf + 1;
		}
		else
#endif
...
#ifdef __thumb__  // deal with ARM/THUMB odd address function boundaries
		if ((long)(&flash_lock_block)&0x01)
		{
			memcpy(_flash_lock_block, (char *)(&flash_lock_block)-1, code_len+1);
			(char *)_flash_lock_block = (char *)_flash_lock_block + 1;
		}
		else
#endif
...
#ifdef __thumb__  // deal with ARM/THUMB odd address function boundaries
		if ((long)(&flash_unlock_block)&0x01)
		{
			memcpy(_flash_unlock_block, (char *)(&flash_unlock_block)-1, code_len+1);
			(char *)_flash_unlock_block = (char *)_flash_unlock_block + 1;
		}
		else
#endif
